...
def user_add(request):
    ...
    if form.is_valid():
        form.save()
        return redirect(memory_reverse(request, 'user_list'))      #反向生成    改成 memory_reverse ，去掉rbac:
...
def user_edit(request, pk):
    ...
    if form.is_valid():
        form.save()
        return redirect(memory_reverse(request, 'user_list'))         #改成 memory_reverse ，去掉rbac:
...
def user_del(request, pk):
    ...
    origin_url = memory_reverse(request, 'user_list')      # delete.html 中取消按钮跳转 origin_url: /rbac/role/list/    改成 memory_reverse ，去掉rbac:
